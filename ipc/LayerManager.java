...
public class LayerManager {
	...
	public void AddLayer(BaseLayer pLayer){		//레이어 추가
		mp_aLayers.add(m_nLayerCount++, pLayer);
		//m_nLayerCount++;
	}
	
	
	public BaseLayer GetLayer(int nindex){		//nindex번째 레이어 반환
		return mp_aLayers.get(nindex);
	}
	
	public BaseLayer GetLayer(String pName){	//이름으로 받아서 레이어 반환
	...
	public void ConnectLayers(String pcList){	//레이어 연결
		MakeList(pcList);
	...
	private void Push (BaseLayer pLayer){	//레이어 스택 푸시
		mp_Stack.add(++m_nTop, pLayer);
		//mp_Stack.add(pLayer);
		//m_nTop++;
	}

	private BaseLayer Pop(){	//레이어 스택 팝
	...
	private BaseLayer Top(){	//Top 원소 반환
		return mp_Stack.get(m_nTop);
	}
	
	private void LinkLayer(_NODE pNode){	//레이어를 링크해준다.
		...
		while(pNode != null){
			if( pLayer == null)
				...
				if(pNode.token.equals("("))
					...
					switch(cMode){
					case '*':	//메인에서 우린 *을 이용해서 레이어들을 연결한다.
