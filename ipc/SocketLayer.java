...
public class SocketLayer implements BaseLayer {
	...
	public SocketLayer(String pName) {	//생성자
		// super(pName);
		pLayerName = pName;

	}

	public void setClientPort(int dstAddress) {	//클라이언트 포트 설정
		this.dst_port = dstAddress; //  참고사항 IPCDlg setting 버튼을 누르면 포트 설정

	}
	
	public void setServerPort(int srcAddress) {	//서버 포트 설정
		this.src_port = srcAddress; // 참고사항 IPCDlg setting 버튼을 누르면 포트 설정
	}

	public boolean Send(byte[] input, int length) {		//input 받아서 보낸다.
